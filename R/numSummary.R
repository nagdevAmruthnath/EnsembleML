#' Provides descriptive statistics for a data frame
#' @param df A data frame
#' @return Returns a table with descriptive statistics for a data frame.
#' @examples
#' ## data(iris)
#' ## numSummary(iris)
#' @export

# provides numerical summary of the a data frame
numSummary = function(df){
  ...
  q         = sapply(DD, function(x) stats::quantile(x, c(.01,.05,.25,.5,.75,.95, .99),na.rm=TRUE) )
